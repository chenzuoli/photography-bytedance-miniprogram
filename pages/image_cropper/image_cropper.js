Component({
  ...
  attached() {
    this.data.info = tt.getSystemInfoSync(); //启用数据监听
    ...
    this.data.imgSrc && (this.data.imgSrc = this.data.imgSrc); //根据开发者设置的图片目标尺寸计算实际尺寸
  ...
  methods: {
    ...
    setTransform(transform) {
      ...
      this.data._img_left = transform.x ? this.data._img_left + transform.x : this.data._img_left; //图像边缘检测,防止截取到空白
      ...
      !this.data._canvas_overflow && this._draw(); //可以居中裁剪框了
    ...
    setCutCenter() {
      ...
      let cut_left = (this.data.info.windowWidth - this.data.width) * 0.5; //顺序不能变
    ...
    pushImg(src) {
      ...
      tt.getImageInfo({
        ...
        success: res => {
          this.data.imageObject = res; //图片非本地路径需要换成本地路径
    ...
    _move(event) {
      ...
      if (event.touches.length == 1) {
        ...
        let left = event.touches[0].clientX - this.data._touch_img_relative[0].x,
            top = event.touches[0].clientY - this.data._touch_img_relative[0].y; //图像边缘检测,防止截取到空白
        ...
        let width = Math.abs(event.touches[0].clientX - event.touches[1].clientX),
            ...
            current_deg = 0;
        scale = scale <= this.data.min_scale ? this.data.min_scale : scale;
        scale = scale >= this.data.max_scale ? this.data.max_scale : scale; //图像边缘检测,防止截取到空白
        ...
        if (!this.data.disable_rotate) {
          ...
          let second_atan_old = 180 / Math.PI * Math.atan2(this.data._touch_img_relative[1].y, this.data._touch_img_relative[1].x); //当前旋转的角度
        ...
        this.data._hypotenuse_length = Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2)); //更新视图
    ...
    _draw(callback) {
      ...
      let draw = () => {
        ...
        let img_height = this.data.img_height * this.data.scale * this.data.export_scale; //canvas和图片的相对距离

        var xpos = this.data._img_left - this.data.cut_left;
        var ypos = this.data._img_top - this.data.cut_top; //旋转画布
    ...
    _cutTouchMove(e) {
      if (this.data._flag_cut_touch && this.data.MOVE_THROTTLE_FLAG) {
        if (this.data.disable_ratio && (this.data.disable_width || this.data.disable_height)) return; //节流
    ...
    _moveDuring() {
      //清空之前的自动居中延迟函数
      clearTimeout(this.data.TIME_CUT_CENTER); //清空之前的背景变化延迟函数

      clearTimeout(this.data.TIME_BG); //高亮背景
